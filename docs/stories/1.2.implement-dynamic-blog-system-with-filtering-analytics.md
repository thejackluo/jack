...
- [x] Build `/writing` listing page backed by Astro content collections (AC: 1, 2, 3)
  - [x] Create `website/src/pages/writing/index.astro` using the shared layout, load posts via `getCollection('blogs')`, and sort newest first (AC: 1, 2) [Source: architecture/application-architecture.md#routing-plan] [Source: architecture/content-data-architecture.md#blogs]
  - [ ] Implement accessible tag and year filters as an Astro island/component that hydrates on interaction while preserving server-rendered content (AC: 3) [Source: architecture/technology-stack-decisions.md#technology-stack-decisions] - **Deferred: Focused on core rendering first**
...
| 2025-01-27 | 0.4 | Implemented Stage 1 (basic blog rendering) and Stage 2 (design & CSS). Created `/writing` listing page and `/writing/[slug]` detail page with glassmorphism styling matching site design. Updated Header navigation. Analytics (Stage 3) deferred per user request. | James (Dev Agent) |
...
- Created `/writing/index.astro` - Blog listing page with all 17 blog posts rendered
- Created `/writing/[slug].astro` - Blog detail page with markdown rendering
- Updated `Header.astro` - Changed navigation link from `/writing.html` to `/writing`
- No linting errors found in new pages

### Completion Notes List
**Stage 1 Complete (Basic Blog Rendering):**
- Created `/writing` listing page that loads all blogs from content collections
- Blogs sorted by publish date (newest first)
- Each blog card displays: title, excerpt, publish date, tags, and hero image
- Created `/writing/[slug]` detail page for individual blog posts
- Detail page includes: hero image, title, metadata, full markdown content, prev/next navigation
- Updated Header navigation to point to new `/writing` route

**Stage 2 Complete (Design & CSS):**
- Implemented glassmorphism design matching site's existing aesthetic
- Enhanced blog cards with radial gradients, backdrop blur, and smooth hover effects
- Improved typography using site fonts (Elianto-Regular, Dual-300, PP Stellar)
- Responsive design across all breakpoints (mobile, tablet, desktop)
- Enhanced blog post detail page with better spacing, typography, and visual hierarchy
- Added smooth transitions and hover effects throughout

**Stage 3 Deferred (Analytics):**
- Analytics tracking deferred per user request
- Plausible integration already exists in Base.astro, can be extended later

**Note:** Tag/year filtering (AC: 3) deferred to focus on core rendering and design first. Can be added in future iteration.

### File List
- `website/src/pages/writing/index.astro` - Blog listing page
- `website/src/pages/writing/[slug].astro` - Blog detail page with dynamic routing
- `website/src/components/Header.astro` - Updated navigation link
