...
| 2025-11-16 | 1.1 | Redesigned blog pages to match original dark aesthetic. Fixed white background to dark gradient (#000000 → #14123c), converted tag/year filters to dropdowns, fixed text blurring on hover, preserved original CSS with glassmorphism effects and Dual-300/Stellar fonts, improved two-column layout and spacing. All functionality preserved with dark theme. | Claude (Dev Agent) |
...
**Design Refinement (v1.1):**
- Redesigned to match original dark aesthetic with gradient background (#000000 → #14123c)
- Applied glassmorphism design pattern from original site to all inputs and cards
- Converted tag and year filters from button lists to dropdown selects for cleaner UI
- Fixed text blurring issue on blog card hover by adding backface-visibility and font-smoothing
- Preserved original fonts: Dual-300 for headings, Stellar/Century Gothic for body text
- Implemented proper two-column layout with Bootstrap grid (col-lg-3 sidebar, col-lg-9 main)
- Added proper spacing between blog entries (2rem gap) and improved card padding
- Limited visible tags to 3 per blog card for cleaner presentation
- Applied white text color scheme throughout with proper opacity levels for visual hierarchy
- Enhanced hover effects with subtle lift and glow instead of problematic blur
- Imported original CSS files (writing.css, blog.css) for consistency
- All functionality preserved: search, filtering, sorting, responsive design, citation removal
